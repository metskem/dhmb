...
func Loop(m db.Monitor) {
	...
	for m.MonStatus == db.MonStatusActive {
		...
		if retries > m.Retries {
			alert(m, statusCode, err)
		}
		time.Sleep(time.Duration(m.Interval) * time.Second)
	}
}

func alert(m db.Monitor, statusCode int, err error) {
	for _, chat := range db.GetChats() {
		_, err := conf.Bot.Send(tgbotapi.NewMessage(chat.ChatId, fmt.Sprintf("%s is down, statusCode: %d, error: %s", m, statusCode, err)))
		if err != nil {
