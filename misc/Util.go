...
func HandleCommand(update tgbotapi.Update) {
	...
	if strings.HasPrefix(update.Message.Text, "/sendPhoto") {
		...
		fileName := strings.Trim(strings.TrimPrefix(update.Message.Text, "/sendPhoto"), " ")
		//photoConfig := tgbotapi.NewPhotoUpload(chatter.ChatId, fileName)
		photoConfig := tgbotapi.NewDocumentUpload(chatter.ChatId, f.Name())
		_, err := Bot.Send(photoConfig)
		if err == nil {
			log.Printf("sent photo %v", photoConfig)
		} else {
			log.Printf("failed sending photo %s, error:%s", fileName, err)
			SendMessage(chatter, err.Error())
		}
	}
